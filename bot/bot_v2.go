...
func (l *botLoader) GetFreeBot(num int) ([]*BotPresence, error) {
	...
	for _, userBot := range ml {
		...
		if err != nil {
			fmt.Printf("maintainChipBalance error for user %s: %v", userBot.User.Id, err)
		}
	}
	return ml, nil
}
...
func (l *botLoader) maintainChipBalance(userId string) error {
	...
	fmt.Printf("maintainChipBalance for user %s, current chip balance: %d", userId, profile.AccountChip)
	if profile.AccountChip < l.minChipBalance {
		fmt.Printf("User %s has chip balance %d, which is less than minimum required %d. Adding chips.", userId, profile.AccountChip, l.minChipBalance)
		ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
